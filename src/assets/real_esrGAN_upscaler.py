...
# NOTE: Do NOT gate this behind an availability check — in a PyInstaller
# frozen app the modules ARE present but may not yet be importable at the
# very top of the entry-point.  We defer the hard ImportError to the first
# actual use (RealESRGANUpscaler.__init__) so PyInstaller's import hooks have
...
def _patch_torchvision() -> None:
    """
    Shim the removed `torchvision.transforms.functional_tensor` sub-module
    that older versions of basicsr still import.  Safe to call multiple times.
    """
    ...
    try:
        ...
        _mock.rgb_to_grayscale = _F.rgb_to_grayscale   # type: ignore[attr-defined]
        sys.modules[key] = _mock
    except ImportError:
        # torchvision not available yet; the patch will be retried when
        # _build_upsampler() is called (torch is guaranteed present by then).
        pass
...
class RealESRGANUpscaler:
    ...
    Heavy dependencies (torch, basicsr, realesrgan) are imported lazily inside
    _build_upsampler() so that:
      • The module can be imported at the top of the app without torch being
        fully initialised (critical for PyInstaller frozen builds).
      • Model weights are loaded on the first upscale call and cached —
    ...
    def __init__(self, config: UpscalerConfig) -> None:
        ...
        # device and upsampler are resolved lazily
    ...
    def upscale_pil(self, image) -> "PIL.Image.Image":  # type: ignore[name-defined]
    ...
    def upscale_numpy(self, img) -> "np.ndarray":
    ...
    def _build_net(self):
        # Heavy imports deferred so PyInstaller hooks are active by the time
        # this is called (always from _build_upsampler, never at import time).
        mc = self.model_cfg
    ...
    def _build_upsampler(self):
        # Re-apply the torchvision shim now that torch is definitely loaded.
        _patch_torchvision()
        ...
        return RealESRGANer(
            ...
            pre_pad=cfg.pre_pad,
            half=(cfg.half and self._device.type == "cuda"),  # type: ignore[union-attr]
            device=self._device,
        )